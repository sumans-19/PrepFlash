import { motion } from "framer-motion";
 ...
 // Each feature with motion/gradient/hover and a 'link' property
 const features = [
  {
   icon: LayoutDashboard,
   title: "AI Interview Simulation",
   description:
    "Intelligent, adaptive interview practice with instant feedback and dynamic question selection.",
   link: "/practice", // Add the navigation link here
  },
  {
   icon: Mic,
   title: "Voice & Video Practice",
   description:
    "Realistic interview experiences with voice and video, including analysis of your speaking tone, clarity, and confidence.",
   link: "/practice", // Add the navigation link here
  },
  {
   icon: Users,
   ...
   description:
    "Connect with other learners, perform mock interviews, share feedback in a safe, supportive environment.",
   link: "/practice", // Add the navigation link here
  },
  {
   icon: Layers,
   ...
   description:
    "Expert-crafted interview prep paths, resources, and progress tracking for every industry.",
   link: "/roadmaps", // Add the navigation link here
  },
  {
   icon: FileText,
   title: "Smart Resume Analysis",
   description: "Upload and optimize your CV/resume with actionable, AI-driven feedback.",
   link: "/resume-analysis", // Add the navigation link here
  },
  {
   icon: Sparkles,
   title: "Tech Trends",
   description:
    "Get insight of current technical trends , emerging technology and News.",
   link: "/tech-news", // Add the navigation link here
  },
 ];
     ...
     <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-9">
      {features.map((feature, index) => (
       ...
       >
        ...
        <h3 className="text-2xl font-semibold mb-2 text-indigo-900 dark:text-fuchsia-200 tracking-tight">
         {feature.title}
        </h3>
        ...
        <Link
         to={feature.link}
         className="flex items-center gap-2 mt-auto font-semibold text-indigo-600 dark:text-fuchsia-300 group-hover:text-fuchsia-600 transition-colors"
        >
         Learn More{" "}
         <ArrowRight className="w-5 h-5 animate-slide-in-right group-hover:translate-x-2 transition-transform" />
        </Link>
